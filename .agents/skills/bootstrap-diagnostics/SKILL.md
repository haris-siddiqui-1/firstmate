...
- `FLEET_SYNC: <repo>: recovered: <detail>` - the sync self-healed a clean checkout holding no unique commits (re-attached and fast-forwarded); no action needed, it is reported only so the self-heal is visible; [`fm-fleet-sync.sh`](../../../bin/fm-fleet-sync.sh) owns the exact recovery preconditions and target.
- `FLEET_SYNC: <repo>: STUCK: ... - needs attention` - the sync left the clone untouched (never forcing or discarding); [`fm-fleet-sync.sh`](../../../bin/fm-fleet-sync.sh) owns the exact unsafe states and quantification, and [architecture](../../../docs/architecture.md) owns the mechanism boundary.
  A loud STUCK, especially a growing behind-count across bootstraps, means that clone needs hands-on attention; dispatch a crewmate or resolve it before it strands work.
